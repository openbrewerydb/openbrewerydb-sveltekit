...
test.describe('Responsive Design', () => {
   ...
   */
  test('breweries page should adapt to different viewport sizes', async ({ page }) => {
   ...
   */
  test('brewery listings should switch between cards and table based on viewport', async ({ page }) => {
  ...
  test('pagination should be responsive', async ({ page }) => {
    ...
    const pagination = page.locator('div:has(> a:text("Next"))');
    ...
    // Verify page counter is visible
    await expect(page.locator('span:text("Page")')).toBeVisible();
  });
});
